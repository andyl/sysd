defmodule RelDep.CLI do
  @moduledoc """
  Escript entry point for the `reldep` command-line tool.

  Parses argv with Optimus, dispatches to `RelDep.Deploy.*`,
  ...
  defp build_optimus do
    Optimus.new!(
      ...
      subcommands: [
        ...
        cleanup: [
          name: "cleanup",
          about: "Remove all RelDep files from a server",
          args: [host: [value_name: "HOST", help: "Server hostname", required: true]],
          options: common_options()
        ],
        setup: [
  ...
  defp load_config(parsed) do
    ...
    case RelDep.Config.load(config_path: config_path) do
      {:ok, config} -> config
  ...
  defp run_check(parsed) do
    ...
    case RelDep.Deploy.check(host, base_opts(parsed)) do
      {:ok, checks} ->
        IO.puts("#{host}:")
  ...
  defp run_deploy(parsed) do
    ...
    case RelDep.Deploy.deploy(host, tarball, opts) do
  ...
  defp run_versions(parsed) do
    ...
    case RelDep.Deploy.versions(host, base_opts(parsed)) do
  ...
  defp run_rollback(parsed) do
    ...
    case RelDep.Deploy.rollback(host, version, base_opts(parsed)) do
  ...
  defp run_status(parsed) do
    ...
    case RelDep.Deploy.status(host, base_opts(parsed)) do
  ...
  defp run_start(parsed) do
    ...
    case RelDep.Deploy.start(host, base_opts(parsed)) do
  ...
  defp run_stop(parsed) do
    ...
    case RelDep.Deploy.stop(host, base_opts(parsed)) do
  ...
  defp run_restart(parsed) do
    ...
    case RelDep.Deploy.restart(host, base_opts(parsed)) do
  ...
  defp run_logs(parsed) do
    ...
    case RelDep.Deploy.logs(host, opts) do
  ...
  defp run_tail(parsed) do
    ...
    case RelDep.Deploy.tail(host, opts) do
      {:ok, output} -> IO.write(output)
      {:error, reason} -> error_exit("Failed: #{inspect(reason)}")
    end
  end
  ...
  defp run_remove(parsed) do
    ...
    case RelDep.Deploy.remove(host, version, base_opts(parsed)) do
  ...
  defp run_cleanup(parsed) do
    ...
    case RelDep.Deploy.cleanup(host, base_opts(parsed)) do
  ...
  defp run_setup(parsed) do
    ...
    case RelDep.Deploy.setup(host, opts) do
  ...
  defp run_systemd(parsed) do
    ...
    output = RelDep.Systemd.render(%{app: app, user: user})
    IO.write(output)
  end
