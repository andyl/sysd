defmodule Mix.Tasks.Ziprel.Setup do
  ...
  Run `mix ziprel.init` first to generate the config and service files,
  then `mix ziprel.sshcheck` to validate connectivity.

  After setup completes, run `mix ziprel.deploy` to perform your first
  deployment.
  """
  use Mix.Task
  ...
  def run(_args) do
    ...
    Mix.shell().info("\nSetup complete. Run `mix ziprel.deploy` to deploy your app.")
  end
end
