defmodule RelDep.SystemdTest do
  use ExUnit.Case

  alias RelDep.Systemd

  describe "render/1" do
    test "renders with required params and defaults" do
      output = Systemd.render(%{app: "myapp", user: "deploy"})

      assert output =~ "# X-Creator=reldep"
      assert output =~ "Description=myapp"
      assert output =~ "User=deploy"
      assert output =~ "WorkingDirectory=/opt/reldep/myapp/current"
      assert output =~ "ExecStart=/opt/reldep/myapp/current/bin/myapp start"
