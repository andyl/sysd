...
Split `mix reldep.deploy` into `mix reldep.release` (build + publish) and
`mix reldep.deploy` (push existing tarball to servers), and introduce a
...
### In scope
- New `mix reldep.release` task (build + optional publish).
- Rework of `mix reldep.deploy` to consume an existing tarball, or
  invoke `mix reldep.release` / fetch from a publisher when absent.
- Publisher contract (behaviour) + dispatcher module.
- `RelDep.Publisher.Github` and `RelDep.Publisher.File` implementations.
- Preflight checks that run before any build work.
- `config/reldep.yaml` schema extension: `release.publish` list, parsed
  into the `%RelDep.Config{}` struct.
...
1. **Publisher as an Elixir behaviour.** Define `RelDep.Publisher` with
   ...
   dispatches via a small `RelDep.Publisher.resolve/1` helper mapping
   `type: "github"` → `RelDep.Publisher.Github`, etc. This matches the
   spec's "keep task code free of per-publisher branching" requirement
   and is the idiomatic Elixir pattern.

2. **Preflight before build.** `reldep.release` runs *all* publisher
...
3. **Config struct extension.** Add `release: %{publish: [...]}` to
   `%RelDep.Config{}`. Parsed publishers are normalized to plain maps
   ...
   round-trip publishers — `reldep.init` stays minimal and users edit
   `reldep.yaml` by hand to add a `release.publish` block (the existing
   `write/1` only ever rewrites the small set of fields it manages,
   e.g. `remove_server`). Document this limitation.

4. **Shared helpers on `RelDep`.**
   - `RelDep.git_sha/0` — reads `git rev-parse HEAD`.
   - `RelDep.git_tag_exists?/1` — `git rev-parse -q --verify refs/tags/<tag>`.
   - `RelDep.build_host/0` — `:inet.gethostname/0`.
   - `RelDep.version_tag/0` — `"v#{version()}"`.
   - `RelDep.tarball_basename/0` — `"#{app}-#{version}.tar.gz"`.
   These centralize small bits the tasks and publishers share.

5. **`reldep.deploy` default path.** When the local tarball is absent
   and `--from-release` is *not* passed, the task calls
   `Mix.Task.run("reldep.release", [])` (build only — publishing will
   still run per config, which matches the spec's "invoke reldep.release"
   wording). If the user wants to deploy *without* publishing, the
   documented path is `mix reldep.release --no-publish && mix reldep.deploy`.
...
6. **`RELEASE_INFO` format.** Plain key=value text file, one field per
   ...
   ```
   Written by uploading a local temp file through `RelDep.SSH.upload/3`,
   then `sudo mv` into the release dir — mirroring the pattern used for
   the systemd service file in `RelDep.Remote.install_service/3`.
...
8. **File publisher existence check for fetch.** For the fetch path,
   ...
   is missing. `reldep.deploy --from-release` walks publishers in config
   order and uses the first that returns a non-error from
   `fetch_supported?/1` (both shipped publishers return `true`).

9. **Tarball location.** `RelDep.release_tar_path/0` already exists and
...
1. **Extend `RelDep.Config` to parse `release.publish`.**
...
2. **Add `RelDep` helper functions.**
   - Files: `lib/reldep.ex`, `test/relman_test.exs`
...
3. **Define the `RelDep.Publisher` behaviour.**
...
4. **Implement `RelDep.Publisher.File`.**
...
5. **Implement `RelDep.Publisher.Github`.**
...
6. **Write `mix reldep.release`.**
   - Files: `lib/mix/tasks/reldep.release.ex` (new), `test/mix/tasks/relman_release_test.exs` (new)
   - Argv parsing with `OptionParser`: `--force`, `--replace`,
     `--no-publish` (all boolean).
   - Flow:
     1. Load config, app, version.
     2. `RelDep.git_tag_exists?(version_tag)` — raise if not.
...
7. **Rework `mix reldep.deploy`.**
   - Files: `lib/mix/tasks/reldep.deploy.ex`, `test/mix/tasks/relman_deploy_test.exs` (new or extended)
   ...
   - Flow:
     ...
     4. If missing and not `--from-release`: `Mix.Task.run("reldep.release", [])`.
...
8. **Add `Remote.write_release_info/4`.**
   ...
   - Called by `reldep.deploy` after `Remote.deploy/4`.

9. **Update `Mix.Tasks.Reldep` help text.**
   - Files: `lib/mix/tasks/reldep.ex`, `test/mix/tasks/relman_test.exs`
   - Add `mix reldep.release` to the task list and flag summary. Update
...
11. **Refresh `reldep.init` template (optional).**
    - Files: `lib/mix/tasks/reldep.init.ex`
    - Add a commented-out `release.publish` example block in the
      generated `config/reldep.yaml`, so users discover the feature.
      Only if `reldep.init` already writes a template; otherwise skip.
...
- Steps 4 and 5 (publisher implementations) can proceed in parallel
  once 3 is in place.
- Step 6 (`reldep.release`) depends on 3, 4, 5.
- Step 7 (`reldep.deploy` rework) depends on 6 and on step 8
  (`RELEASE_INFO` writer).
...
- **Unit-testable modules** — config parser, pure helpers on `RelDep`,
...
- **Stubbed publisher for task tests.** Add a tiny `TestPublisher`
  ...
  `RelDep.Publisher.resolve/1` can pick it up when a `type: :test`
  spec appears. Use it to drive end-to-end tests of `reldep.release`
  and `reldep.deploy` without touching `gh` or SSH.
...
- **Manual verification checklist** (to run before merging):
  1. `mix reldep.release` with no `release.publish` — builds only.
  2. `mix reldep.release` with `type: file` — copy lands in target dir.
  3. `mix reldep.release` with `type: github` against a scratch repo.
  4. `mix reldep.release --force` after a successful build.
  5. `mix reldep.release --replace` against an existing GH release.
  6. `mix reldep.deploy` with a pre-built tarball.
  7. `mix reldep.deploy` with no tarball (falls through to release).
  8. `git checkout <tag>` in a fresh worktree →
     `mix reldep.deploy --from-release` fetches from file publisher.
...
- [x] Should `reldep.deploy` (no flag) propagate `--no-publish` when
      it invokes `reldep.release`, or should it always publish?
      Current plan: always publish (matches the spec's literal wording
      "invoke `mix reldep.release`"). Confirm this is the intended UX.  Answer: always publish
- [x] Should `reldep.init` grow a commented publisher example now or
...
- [x] `MIX_ENV=prod` — the existing `reldep.deploy` task does not set
      it explicitly. Keep parity (document precondition) or set it
      inside the new `reldep.release` task? Plan currently keeps parity.  Answer: good question.  I'll use "MIX_ENV=prod mix reldep.release; MIX_ENV=prod mix reldep.deploy"
