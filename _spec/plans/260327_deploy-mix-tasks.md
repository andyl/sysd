...
Implement the full set of Ziprel Mix tasks (`ziprel`, `ziprel.init`, `ziprel.sshcheck`, `ziprel.setup`, `ziprel.deploy`, `ziprel.versions`, `ziprel.rollback`, `ziprel.remove`, `ziprel.cleanup`) to provide a complete CLI for deploying Elixir Releases to bare metal servers over SSH.
...
1. **Core module `Ziprel`** — Replace the placeholder with shared logic: config loading, SSH connection management, remote command execution helpers, and constants (paths, directory layout).

2. **Config module `Ziprel.Config`** — Responsible for reading and writing `config/ziprel.yaml`. A YAML library dependency is needed (e.g. `yaml_elixir`). The config struct holds `servers` (list of hostnames) and `ssh` (map with `user` key, extensible for port/key options).

3. **SSH module `Ziprel.SSH`** — Wraps SSHEx for command execution and Erlang's `:ssh_sftp` for file uploads. SSHEx does not support SCP/SFTP, so file transfer must use `:ssh_sftp.write_file/3` or `:ssh_sftp.start_channel/2` directly. This module provides `connect/2`, `run!/3`, `upload/4` functions.

4. **Remote module `Ziprel.Remote`** — Higher-level operations on a connected server: create directories, deploy archives, manage symlinks, control systemd services, list versions. This keeps Mix tasks thin.

5. **Mix tasks under `lib/mix/tasks/`** — Each task is a thin wrapper that loads config, iterates over servers, and delegates to `Ziprel.Remote`. Tasks use `Mix.shell().info/1` for output.

6. **EEX template for systemd** — Shipped as `priv/ziprel/templates/app.service.eex` within the ziprel package itself. `ziprel.init` copies a rendered version into the consumer's `priv/ziprel/<appname>.service`.
...
2. **Create `Ziprel.Config` module**
   - Files: `lib/ziprel/config.ex`
   - Struct: `%Config{servers: [], ssh: %{user: nil}}`
   - `load/0` — reads `config/ziprel.yaml` from the consumer project root, parses YAML, returns `%Config{}`
...
3. **Create `Ziprel.SSH` module**
   - Files: `lib/ziprel/ssh.ex`
...
7. **Implement `mix ziprel` task (help)**
   - Files: `lib/mix/tasks/ziprel.ex`
   - Prints usage overview listing all tasks and one-line descriptions

8. **Implement `mix ziprel.init` task**
   - Files: `lib/mix/tasks/ziprel.init.ex`
   - Generates `config/ziprel.yaml` with default content if it doesn't exist
   - Generates `priv/ziprel/<appname>.service` from EEX template
...
10. **Implement `mix ziprel.setup` task**
    - Files: `lib/mix/tasks/ziprel.setup.ex`
    - Loads config, iterates servers
    - For each: connect, call `Remote.setup_dirs/1`, call `Remote.install_service/3`
    - Then invoke the deploy workflow

11. **Implement `mix ziprel.deploy` task**
    - Files: `lib/mix/tasks/ziprel.deploy.ex`
...
12. **Implement `mix ziprel.versions` task**
    - Files: `lib/mix/tasks/ziprel.versions.ex`
    - Loads config, iterates servers
    - For each: connect, call `Remote.list_versions/1`, print results

13. **Implement `mix ziprel.rollback` task**
    - Files: `lib/mix/tasks/ziprel.rollback.ex`
    - Parse version argument from CLI args
    - Loads config, iterates servers
    - For each: connect, call `Remote.rollback/2`

14. **Implement `mix ziprel.remove` task**
    - Files: `lib/mix/tasks/ziprel.remove.ex`
    - Parse version argument from CLI args
    - Loads config, iterates servers
    - For each: connect, check current version, call `Remote.remove_version/2`

15. **Implement `mix ziprel.cleanup` task**
    - Files: `lib/mix/tasks/ziprel.cleanup.ex`
...
16. **Update tests**
    - Files: `test/ziprel_test.exs`, `test/ziprel/config_test.exs`
    - Remove placeholder `hello` test
    - Add unit tests for `Ziprel.Config` (load/write/remove_server with fixture YAML files)
    - Add unit tests for `Ziprel` helper functions (app_name, version, release_tar_path)
...
- **Step 6** (refactor Ziprel module) should precede task implementations
...
- **Config file not found**: Tasks that require config should print a clear error suggesting `mix ziprel.init`
- **Removing current version**: `ziprel.remove` must check the symlink target before deleting
- **Cleanup of running service**: `ziprel.cleanup` should stop the service before removing files
- **YAML formatting**: `yaml_elixir` encoding should produce clean, readable YAML matching the spec format

## Testing Strategy

- **Unit tests** for `Ziprel.Config`: read/write/parse YAML fixtures, test validation errors, test `remove_server`
- **Unit tests** for `Ziprel` helpers: verify app name, version, tarball path derivation
- **Unit tests** for `Ziprel.Remote` command building: verify the SSH commands generated for each operation (mock the SSH layer)
