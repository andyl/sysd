...
Ship a standalone `reldep` escript binary and refactor all deploy logic into a shared `RelDep.Deploy.*` library layer with a strict no-IO/no-halt rule, so the escript, Mix tasks, and Mix.install scripts are thin wrappers over the same core.

## Scope

### In scope
- Library refactor: extract `RelDep.Deploy.*` modules from existing `RelDep.Remote` + Mix task logic
- `RelDep.TarballRef` module for parsing tarball reference schemes (`file://`, `gh://`, bare paths; `s3://` stub)
- `RelDep.Config.load/1` with four-level precedence chain (CLI flag, env var, project-local walk-up, XDG)
- Systemd template as a module attribute with `X-Creator=reldep` marker
- Escript entry point using Optimus for subcommand parsing
- `mix escript.build` configuration in `mix.exs`
- Refactor existing Mix tasks to delegate to `RelDep.Deploy.*`
...
- Changes to publisher logic (`RelDep.Publisher.*`)
- GitHub release automation for the escript binary

## Architecture & Design Decisions

1. **Library-first refactor.** All operations move to `RelDep.Deploy` (facade) backed by focused sub-modules. The existing `RelDep.Remote` module stays as the SSH command layer; `RelDep.Deploy.*` composes Remote + Config + TarballRef into high-level operations returning `{:ok, result} | {:error, reason}` tuples.

2. **No IO/halt rule.** `RelDep.Deploy.*` modules must never call `IO.puts`, `Mix.shell()`, `System.halt`, or `Mix.raise`. Errors are returned as `{:error, reason}` tuples. The callers (escript CLI, Mix tasks) handle output and exit codes.

3. **`RelDep.SSH.run!` currently raises `Mix.Error`.** This needs to change: the bang functions in SSH should raise a generic `RelDep.SSH.Error` (or return error tuples used by Deploy), since `Mix.raise` is not available in escript context. Mix tasks can catch and re-raise as `Mix.Error`.

4. **Optimus for CLI parsing.** Optimus provides subcommand support, help text generation, and argument/flag validation out of the box. Each subcommand maps 1:1 to a `RelDep.Deploy` function.

5. **Config precedence chain.** `RelDep.Config.load/1` accepts an optional explicit path (from `--config` flag). When nil, it checks `RELDEP_CONFIG` env var, then walks up from cwd looking for `reldep.yml`, then falls back to `~/.config/reldep/config.yml`. The existing `RelDep.Config.load/0` (hardcoded `config/reldep.yaml`) is preserved for backward compat in Mix tasks but delegates to the new logic.

6. **Tarball ref parsing.** `RelDep.TarballRef.parse/1` returns a tagged struct: `%TarballRef{scheme: :file | :github | :s3, ...}`. A `resolve/2` function materializes the ref to a local file path (downloading if needed). `s3://` returns `{:error, :not_implemented}`.

7. **Systemd template.** Embedded as a module attribute in `RelDep.Systemd`, using EEx compilation. Parameterized by a map of keys. Includes `# X-Creator=reldep` marker comment.
...
2. **Create `RelDep.SSH.Error` and refactor SSH error handling**
   - Files: `lib/reldep/ssh.ex`
   - Define `defexception` for `RelDep.SSH.Error`
   - Change `SSH.run!/2` to raise `RelDep.SSH.Error` instead of `Mix.raise`
   - Change `SSH.upload/3` to return `{:ok, :uploaded} | {:error, reason}` instead of calling `IO.puts`

3. **Create `RelDep.TarballRef` module**
   - Files: `lib/reldep/tarball_ref.ex`
...
4. **Refactor config loading with precedence chain**
   - Files: `lib/reldep/config.ex`
   - Add `load/1` that accepts `opts` keyword list with optional `:config_path`
   - Implement precedence: explicit path → `RELDEP_CONFIG` env → walk-up discovery of `reldep.yml` → XDG `~/.config/reldep/config.yml`
...
5. **Create `RelDep.Systemd` module**
   - Files: `lib/reldep/systemd.ex`
   - Module attribute `@service_template` with EEx template string including `# X-Creator=reldep`
...
6. **Create `RelDep.Deploy` facade module**
   - Files: `lib/reldep/deploy.ex`
   ...
   - Composes `RelDep.SSH`, `RelDep.Remote`, `RelDep.Config`, `RelDep.TarballRef`
...
7. **Add missing remote operations to `RelDep.Remote`**
   - Files: `lib/reldep/remote.ex`
...
8. **Refactor existing Mix tasks to use `RelDep.Deploy`**
   - Files: `lib/mix/tasks/reldep.deploy.ex`, `reldep.sshcheck.ex`, `reldep.versions.ex`, `reldep.rollback.ex`, `reldep.remove.ex`, `reldep.cleanup.ex`, `reldep.setup.ex`
   - Each task becomes: parse args → call `RelDep.Deploy.<function>` → pattern-match result → print output or raise
   - This validates the library API before building the escript

9. **Create escript entry point with Optimus**
   - Files: `lib/reldep/cli.ex`
   - `main/1` — parse argv with Optimus, dispatch to `RelDep.Deploy.*`, format output, set exit code
...
10. **Configure `mix escript.build` in mix.exs**
    - Files: `mix.exs`
    - Add `escript: [main_module: RelDep.CLI, name: "reldep"]` to project config
    - Ensure `:yaml_elixir` and `:sshex` are included in the escript

11. **Write tests for new modules**
    - Files: `test/reldep/tarball_ref_test.exs`, `test/reldep/deploy_test.exs`, `test/reldep/systemd_test.exs`, `test/reldep/config_test.exs` (extend)
...
12. **Verify escript builds and runs**
    - Run `mix escript.build` and test `./reldep --help`, `./reldep deploy --help`, etc.
...
- **`RelDep.app_name/0` and `RelDep.version/0` call `Mix.Project.config`**: These won't work in escript mode. The escript must get app name from CLI args or config, not from Mix. The Deploy facade should accept app/version as explicit params.
...
- **Config backward compatibility**: Existing projects use `config/reldep.yaml` with a flat structure. The new multi-app config shape (`apps:` key) is different. Config parsing must handle both formats.
...
- **Integration-style tests** for `RelDep.Deploy.*` — mock SSH at the `RelDep.SSH` boundary using a behaviour + test double
- **Escript smoke test** — `mix escript.build && ./reldep --help` returns 0 with expected output
