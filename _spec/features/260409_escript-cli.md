...
Add a standalone escript binary (`reldep`) that exposes all deployment
operations as CLI subcommands. The escript, Mix tasks, and Mix.install scripts
all become thin wrappers over a shared library core (`RelDep.Deploy.*`),
...
- Ship a standalone `reldep` escript installable system-wide, requiring only
  the Erlang runtime.
- Refactor deploy internals into `RelDep.Deploy.*` modules with a strict
...
- Support a four-level configuration precedence chain: CLI flags, environment
  variable, project-local `reldep.yml`, user-level XDG config.
- Embed a systemd service template as a module attribute with an
  `X-Creator=reldep` marker comment.
...
The `<tarball-ref>` argument to `reldep deploy` accepts:
...
Parsing is handled by a dedicated `RelDep.TarballRef` module.
...
Config loading is handled by `RelDep.Config.load/1`.

## Library Refactor

All deploy operations move into `RelDep.Deploy.*` modules with a public API
returning `{:ok, result}` / `{:error, reason}` tuples. No `IO.puts` or
`System.halt` calls are permitted in `RelDep.Deploy.*`. Output formatting and
...
`exec_start`, `environment`, `restart`, `restart_sec`. An `X-Creator=reldep`
marker comment identifies services managed by reldep.
